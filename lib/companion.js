...
const BANK_RESPONSES = [
  ...
  {
    keywords: ["fraud", "scam", "unknown", "unauthorized", "stolen", "phishing"],
    reply: {
...
export const COMPANION_PROMPTS = {
  gov: [
    "What documents for birth certificate?",
    "What does income certificate mean?",
    "Where do I apply for ration card?",
  ],
  bank: [
    "What is KYC?",
    "I got an unknown UPI debit",
...
export function findCompanionFallback(message, domain, language = "en") {
  ...
  if (!trimmed) {
    return pickLang(DEFAULT_REPLIES[domain === "bank" ? "bank" : "gov"], language);
  }

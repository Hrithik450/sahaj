...
export async function playFeatureIntro(
  domainKey,
  featureId,
  language = "en",
) {
  await playVoiceGuide({
    language,
    audioPath: featureVoiceAudioPath(domainKey, featureId, language),
    text: featureVoiceCaption(domainKey, featureId, language),
    style: featureVoiceStyle(domainKey, featureId, language),
  });
}
